...
now_dir = os.path.abspath('./01_pcx_raw_data')
# 해당 경로의 파일들을 불러오는 과정
for f in os.listdir('./01_pcx_raw_data'):
...
# 파일이름으로 저장
nw.save(f'./00_upload_file/{file_name}')
...
# 파일이름으로 저장
nw.save(f'./02_data_set/{file_name}')
...
bd_list = os.listdir('./02_data_set')
xlist = list()
...
dfs = pd.read_excel(f'./02_data_set/{now_file}', sheet_name=None )
odfs = pd.read_excel(f'./02_data_set/{recent_file}', sheet_name=None )
...
# 이미지 파일 만들 폴더 생성
img_dir = file_name[:-5]
os.makedirs(f'./03_charts/{img_dir}')
img_dir = f'./03_charts/{img_dir}'
